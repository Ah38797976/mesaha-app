...
function CoupleSetupScreen({ c, userId, onLinked }) {
  ...
  const create = async () => {
    setErr(''); setBusy(true);
    try {
      // حماية: لو المستخدم عضو بمساحة أصلاً (مثلاً ضغط الزر مرتين)، نربطه بيها بدل إنشاء وحدة مكررة
      const { data: existing } = await supabase.from('couple_members').select('couple_id').eq('user_id', userId).limit(1);
...
function ChatScreen({ c, coupleId, me, partner }) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      alert('تعذر الوصول للمايكروفون — تأكد من إعطاء الإذن للموقع من إعدادات المتصفح');
      return;
    }
    // Safari على الآيفون ما يدعم webm — نكتشف الصيغة المدعومة فعليًا بدل ما نجبر نوع غلط
    ...
    const actualType = rec.mimeType || mimeType || 'audio/mp4';
    ...
    rec.onstop = async () => {
      try {
        ...
        const path = await uploadBlob('voice-notes', coupleId, blob, ext, 'voice-');
        ...
        alert('تعذر إرسال الرسالة الصوتية: ' + (e.message || 'خطأ غير معروف'));
      } finally {
...
export default function MesahaApp() {
  ...
  useEffect(() => {
    supabase.auth.getSession()
      .then(({ data, error }) => {
        if (error) { console.error('[getSession]', error); setBootError(error.message); }
        setSession(data.session ?? null);
      })
      .catch((e) => { console.error('[getSession] threw', e); setBootError(e.message); setSession(null); });
  ...
  const loadMe = useCallback(async (userId) => {
    ...
    if (!error) { setMe(data); return; }
    // الصف غير موجود (مثلاً بسبب تأكيد البريد الذي أخّر إنشاء الجلسة أثناء التسجيل) — ننشئه تلقائيًا
    if (error.code === 'PGRST116') {
      const { data: authUser } = await supabase.auth.getUser();
      const fallbackName = authUser?.user?.email?.split('@')[0] || 'مستخدم';
      const { data: created, error: insErr } = await supabase.from('profiles')
        .insert({ id: userId, display_name: fallbackName, country: '' })
        .select('*').single();
      if (insErr) { console.error('[loadMe:auto-create]', insErr); setBootError(insErr.message); return; }
      setMe(created);
      return;
    }
    console.error('[loadMe]', error); setBootError(error.message);
  }, []);

  const loadCouple = useCallback(async (userId) => {
    // limit(1) بدل maybeSingle: ما ينكسر حتى لو صار عندنا أكثر من صف بالغلط (تكرار قديم)
    const { data: memberships, error: mErr } = await supabase.from('couple_members')
      .select('couple_id').eq('user_id', userId).order('joined_at', { ascending: true }).limit(1);
    if (mErr) { console.error('[loadCouple:membership]', mErr); setBootError(mErr.message); return; }
    ...
    if (cErr) { console.error('[loadCouple:couples]', cErr); setBootError(cErr.message); return; }
    setCouple(coupleRow);
    const { data: members, error: mmErr } = await supabase.from('couple_members').select('user_id').eq('couple_id', membership.couple_id);
    if (mmErr) { console.error('[loadCouple:members]', mmErr); }
    const partnerId = members?.find((m) => m.user_id !== userId)?.user_id;
    if (partnerId) {
      const { data: pProfile, error: pErr } = await supabase.from('profiles').select('*').eq('id', partnerId).single();
      if (pErr) console.error('[loadCouple:partner]', pErr);
      setPartner(pProfile);
    } else {
      setPartner(null);
    }
  ...
  if (session === undefined || (session && !me)) {
    body = (
      <div style={{ background: c.bg, color: c.textDim }} className="h-full w-full flex flex-col items-center justify-center text-sm gap-3 px-6 text-center">
        <div>...جاري التحميل</div>
        {bootError && (
          <>
            <div style={{ color: c.rose }} className="text-xs">
              تعذّر الاتصال: {bootError}
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setBootError(null); setSession(undefined); supabase.auth.getSession().then(({ data }) => setSession(data.session ?? null)); }}
                style={{ background: c.goldSoft, color: c.gold, border: `1px solid ${c.border}` }}
                className="rounded-lg px-3 py-2 text-xs">إعادة المحاولة</button>
              <button onClick={() => { supabase.auth.signOut(); setSession(null); setMe(null); }}
                style={{ background: c.bg3, color: c.textDim, border: `1px solid ${c.border}` }}
                className="rounded-lg px-3 py-2 text-xs">تسجيل الخروج</button>
            </div>
          </>
        )}
      </div>
    ...
    body = (
      <div className="h-full flex flex-col">
        <div className="flex-1 min-h-0">
          ...
          {tab === 'settings' && <SettingsScreen c={c} dark={dark} setDark={() => {}} lang={lang} setLang={() => {}} me={me} coupleId={couple.id} partner={partner} />}
        </div>
  ...
  return (
    <div dir="rtl" style={{ fontFamily: FONT_BODY }} className="w-full h-screen flex items-center justify-center">
      ...
      <div style={{
        ...
        {bootError && me && (
          <div style={{ background: '#3a1620', color: '#F1E4C6', border: `1px solid ${c.rose}` }}
            className="absolute top-2 inset-x-2 z-50 rounded-xl p-3 text-xs flex items-center justify-between gap-2">
            <span>خطأ: {bootError}</span>
            <button onClick={() => { setBootError(null); loadCouple(session.user.id); }}
              style={{ color: c.rose }} className="underline whitespace-nowrap">إعادة</button>
          </div>
        )}
        {body}
      </div>

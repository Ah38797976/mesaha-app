...
import {
  ...
  HelpCircle, Shuffle, Bookmark, LogOut, Copy, Link2, Square, Repeat2, SmilePlus,
  Plus, Video, Paperclip, Sticker, FileText, Download, HeartHandshake, Gamepad2
} from 'lucide-react';
...
// The 6 quick-reaction emojis available on every message bubble.
...
function ChatScreen({ c, coupleId, me, partner }) {
  ...
  const [reactions, setReactions] = useState([]); // flat list of {id, message_id, user_id, emoji}
  ...
  const loadReactions = useCallback(async () => {
    const { data } = await supabase.from('message_reactions').select('*').eq('couple_id', coupleId);
    setReactions(data || []);
  }, [coupleId]);

  useEffect(() => { loadMessages(); loadSecrets(); loadReactions(); }, [loadMessages, loadSecrets, loadReactions]);
  ...
  useEffect(() => {
    const ch = supabase.channel(`messages-${coupleId}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'message_reactions', filter: `couple_id=eq.${coupleId}` },
        (payload) => {
          if (payload.eventType === 'INSERT') setReactions((r) => [...r, payload.new]);
          if (payload.eventType === 'UPDATE') setReactions((r) => r.map((x) => x.id === payload.new.id ? payload.new : x));
          if (payload.eventType === 'DELETE') setReactions((r) => r.filter((x) => x.id !== payload.old.id));
        })
      .subscribe();
  ...
  // One shared uploader for every attach-menu option (image/video/file/gif).
  // `type` becomes messages.type; extra file metadata is only stored for
  // generic files so the bubble can show a filename + size.
  const uploadAndSendMedia = async (file, type, prefix) => {
    if (!file) return;
    if (file.size > MAX_FILE_MB * 1024 * 1024) {
      alert(`الملف حجمه أكبر من ${MAX_FILE_MB} ميجا، اختر ملف أصغر 🙏`);
      return;
    }
    setUploading(true);
    try {
      const path = await uploadToBucket('chat-media', coupleId, file, prefix);
      const extra = type === 'file' ? { file_name: file.name, file_size: file.size } : {};
      await supabase.from('messages').insert({ couple_id: coupleId, sender_id: me.id, type, media_path: path, ...extra });
      const labels = { image: 'صورة 📸', video: 'فيديو 🎬', file: 'ملف 📎', gif: 'GIF 🎞️' };
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'message', body: `${me.display_name} أرسل ${labels[type]}` });
    } catch (err) {
      alert('تعذر الإرسال، حاول مرة أخرى.');
      console.error(err);
    } finally {
      setUploading(false);
    }
  ...
  const pickImage = () => { setShowAttachMenu(false); fileInputRef.current?.click(); };
  const pickVideo = () => { setShowAttachMenu(false); videoInputRef.current?.click(); };
  const pickAnyFile = () => { setShowAttachMenu(false); fileAnyInputRef.current?.click(); };
  const pickGif = () => { setShowAttachMenu(false); gifInputRef.current?.click(); };

  const onImageChosen = (e) => { const f = e.target.files?.[0]; e.target.value = ''; uploadAndSendMedia(f, 'image', 'img-'); };
  const onVideoChosen = (e) => { const f = e.target.files?.[0]; e.target.value = ''; uploadAndSendMedia(f, 'video', 'vid-'); };
  const onAnyFileChosen = (e) => { const f = e.target.files?.[0]; e.target.value = ''; uploadAndSendMedia(f, 'file', 'doc-'); };
  const onGifChosen = (e) => { const f = e.target.files?.[0]; e.target.value = ''; uploadAndSendMedia(f, 'gif', 'gif-'); };
  ...
  // Tapping an emoji: first tap sets/changes your reaction on that message,
  // tapping the SAME emoji you already reacted with removes it.
  const toggleReaction = async (messageId, emoji) => {
    const mine = reactions.find((r) => r.message_id === messageId && r.user_id === me.id);
    ...
    await supabase.from('message_reactions').insert({ couple_id: coupleId, message_id: messageId, user_id: me.id, emoji });
    const msg = messages.find((m) => m.id === messageId);
    if (msg && msg.sender_id !== me.id && partner) {
      notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'reaction', body: `${me.display_name} تفاعل مع رسالتك ${emoji}` });
    }
  };

  // "إعادة مشاركة": re-send an existing message's content as a brand-new
  // message at the bottom of the chat (works for text, images and voice).
  const reshareMsg = async (m) => {
    setActiveMsgMenu(null);
    const { data } = await supabase.from('messages').insert({
      couple_id: coupleId, sender_id: me.id, type: m.type,
      text: m.text || null, media_path: m.media_path || null,
      voice_duration_seconds: m.voice_duration_seconds || null,
      file_name: m.file_name || null, file_size: m.file_size || null,
      card_kind: m.card_kind || null, card_payload: m.card_payload || null,
    }).select().single();
    if (data && partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'message', body: `${me.display_name} شارك رسالة معك ❤️` });
  };
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <input ref={videoInputRef} type="file" accept="video/*" className="hidden" onChange={onVideoChosen} />
      <input ref={fileAnyInputRef} type="file" className="hidden" onChange={onAnyFileChosen} />
      <input ref={gifInputRef} type="file" accept="image/gif" className="hidden" onChange={onGifChosen} />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4" style={{ background: c.bg }}>
        ...
        {filtered.map((m) => {
          ...
          const myReaction = msgReactions.find((r) => r.user_id === me.id);
          // group reactions by emoji -> count, for the little badge row under the bubble
          ...
          return (
            <div key={m.id} className={`flex flex-col mb-3 ${mine ? 'items-end' : 'items-start'}`}>
              <div className={`flex ${mine ? 'justify-end' : 'justify-start'} relative`}
                onClick={() => { setActiveMsgMenu(activeMsgMenu === m.id ? null : m.id); setReactionPickerFor(null); }}>
                <div style={{
                  ...
                  {m.reply_to_id && (
                    <div style={{ borderInlineStart: `2px solid currentColor`, opacity: 0.75 }} className="text-xs mb-1 ps-2 truncate max-w-[220px]">
                      {messages.find((x) => x.id === m.reply_to_id)?.text || 'وسائط'}
                    </div>
                  )}
                  ...
                  {m.type === 'video' && (
                    mediaUrls[m.media_path]
                      ? <video controls src={mediaUrls[m.media_path]} className="w-48 rounded-xl" style={{ maxHeight: 220 }} />
                      : <div style={{ background: c.bg3 }} className="w-48 h-28 rounded-xl flex items-center justify-center"><Video size={26} style={{ color: c.textDim }} /></div>
                  )}
                  {m.type === 'gif' && (
                    mediaUrls[m.media_path]
                      ? <img src={mediaUrls[m.media_path]} alt="GIF" className="w-40 h-28 rounded-xl object-cover" />
                      : <div style={{ background: c.bg3 }} className="w-40 h-28 rounded-xl flex items-center justify-center"><Sticker size={26} style={{ color: c.textDim }} /></div>
                  )}
                  {m.type === 'file' && (
                    <a href={mediaUrls[m.media_path] || undefined} target="_blank" rel="noreferrer"
                      className="flex items-center gap-2 py-1 min-w-[160px]">
                      <FileText size={22} />
                      <div className="flex-1 overflow-hidden">
                        <div className="text-xs truncate">{m.file_name || 'ملف'}</div>
                        {m.file_size ? <div className="text-[10px] opacity-70">{Math.round(m.file_size / 1024)} كيلوبايت</div> : null}
                      </div>
                      <Download size={16} />
                    </a>
                  )}
                  {m.type === 'card' && <MessageCard c={c} kind={m.card_kind} payload={m.card_payload} mine={mine} />}
                ...
                {reactionPickerFor === m.id && (
                  <div onClick={(e) => e.stopPropagation()} style={{ background: c.bg2, border: `1px solid ${c.border}` }}
                    className={`absolute -top-11 ${mine ? 'right-0' : 'left-0'} flex gap-1 rounded-full px-2 py-1.5 shadow-lg z-10`}>
                    {REACTION_EMOJIS.map((em) => (
                      <button key={em} onClick={() => toggleReaction(m.id, em)}
                        className="text-lg leading-none active:scale-125 transition-transform"
                        style={{ opacity: myReaction?.emoji === em ? 1 : 0.85 }}>{em}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {Object.keys(grouped).length > 0 && (
                <div className={`flex gap-1 mt-1 ${mine ? 'me-1' : 'ms-1'}`}>
                  {Object.entries(grouped).map(([em, count]) => (
                    <button key={em} onClick={() => toggleReaction(m.id, em)} style={{
                      background: c.bg2, border: `1px solid ${myReaction?.emoji === em ? c.gold : c.border}`,
                    }} className="rounded-full px-1.5 py-0.5 text-xs flex items-center gap-0.5">
                      <span>{em}</span>{count > 1 && <span style={{ color: c.textDim }}>{count}</span>}
                    </button>
                  ))}
                </div>
              )}
              ...
              {activeMsgMenu === m.id && (
                <div style={{ background: c.bg2, border: `1px solid ${c.border}` }}
                  className="flex gap-3 mt-1 px-3 py-2 rounded-xl text-xs">
                  <button onClick={(e) => { e.stopPropagation(); setReactionPickerFor(m.id); setActiveMsgMenu(null); }}
                    className="flex items-center gap-1" style={{ color: c.gold }}><SmilePlus size={14} /> تفاعل</button>
                  <button onClick={(e) => { e.stopPropagation(); setReplyTo(m); setActiveMsgMenu(null); }}
                    className="underline" style={{ color: c.text }}>رد</button>
                  <button onClick={(e) => { e.stopPropagation(); reshareMsg(m); }}
                    className="flex items-center gap-1" style={{ color: c.text }}><Repeat2 size={14} /> إعادة مشاركة</button>
                  {mine && <button onClick={(e) => { e.stopPropagation(); deleteMsg(m.id); }}
                    className="underline" style={{ color: c.rose }}>حذف</button>}
                </div>
              )}
            </div>
          );
      ...
      <div style={{ background: c.bg2, borderTop: `1px solid ${c.border}` }} className="px-3 py-3 flex items-center gap-2 relative">
        {showAttachMenu && (
          <div onClick={(e) => e.stopPropagation()} style={{ background: c.bg2, border: `1px solid ${c.border}` }}
            className="absolute bottom-14 start-3 grid grid-cols-4 gap-3 rounded-2xl p-3 shadow-lg z-10">
            <button onClick={pickImage} className="flex flex-col items-center gap-1 text-[10px]" style={{ color: c.text }}>
              <span style={{ background: c.goldSoft, color: c.gold }} className="w-11 h-11 rounded-full flex items-center justify-center"><ImageIcon size={19} /></span>صورة
            </button>
            <button onClick={pickVideo} className="flex flex-col items-center gap-1 text-[10px]" style={{ color: c.text }}>
              <span style={{ background: c.goldSoft, color: c.gold }} className="w-11 h-11 rounded-full flex items-center justify-center"><Video size={19} /></span>فيديو
            </button>
            <button onClick={pickGif} className="flex flex-col items-center gap-1 text-[10px]" style={{ color: c.text }}>
              <span style={{ background: c.goldSoft, color: c.gold }} className="w-11 h-11 rounded-full flex items-center justify-center"><Sticker size={19} /></span>GIF
            </button>
            <button onClick={pickAnyFile} className="flex flex-col items-center gap-1 text-[10px]" style={{ color: c.text }}>
              <span style={{ background: c.goldSoft, color: c.gold }} className="w-11 h-11 rounded-full flex items-center justify-center"><Paperclip size={19} /></span>ملف
            </button>
          </div>
        )}
        <button onClick={() => setShowAttachMenu((s) => !s)} disabled={uploading}
          style={{ color: c.gold, opacity: uploading ? 0.5 : 1 }}>
          {uploading ? <span className="text-[10px]">…</span> : <Plus size={22} />}
        </button>
...
// A generic "rich card" used for any shared content dropped into the chat
// (question / memory / story / phrase / game). `payload` is the free-form
// JSON stored on the message — each kind reads the fields it needs and
// ignores the rest, so future card kinds can add fields without migrations.
function MessageCard({ c, kind, payload = {}, mine }) {
  const KIND_META = {
    question: { icon: HelpCircle, label: 'سؤال من مجلسنَ' },
    memory: { icon: Sparkles, label: 'ذكرى' },
    story: { icon: BookOpen, label: 'قصتنا' },
    phrase: { icon: Heart, label: 'كلمة حب' },
    game: { icon: Gamepad2, label: 'من بيناتنا' },
  };
  const meta = KIND_META[kind] || { icon: HeartHandshake, label: 'مشاركة' };
  const Icon = meta.icon;
  return (
    <div style={{ background: mine ? 'rgba(0,0,0,0.12)' : c.bg3, border: `1px solid ${mine ? 'rgba(0,0,0,0.18)' : c.border}` }}
      ...
      {payload.title && <div className="text-sm font-bold mb-0.5">{payload.title}</div>}
      {payload.date && <div className="text-[10px] opacity-70 mb-1">{payload.date}</div>}
      {payload.text && <div className="text-xs leading-relaxed line-clamp-3">{payload.text}</div>}
    </div>
  );
}

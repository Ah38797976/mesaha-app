...
import {
  ...
  HelpCircle, Shuffle, Bookmark, LogOut, Copy, Link2, Square, Film, ImagePlus, Trash2, Share2
} from 'lucide-react';
...
/* ---------------------------------------------------------
   compressImage — shrinks big phone-camera photos client-side
   before upload. This is the main fix for slow loading: raw
   photos can be 4-8MB, which is slow to upload AND slow for
   the partner to download. We resize to a sane max dimension
   and re-encode as JPEG. GIFs are skipped so animation survives.
--------------------------------------------------------- */
...
/* ---------------------------------------------------------
   ImageLightbox — full-screen viewer with smooth swipe/arrow
   navigation between a list of image urls. Used by both the
   chat and the memories gallery so browsing photos feels fast
   and native, not like separate static bubbles.
--------------------------------------------------------- */
function ImageLightbox({ images, startIndex = 0, onClose }) {
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === 'ArrowLeft') go(1);   // RTL: visually-left arrow = next in reading order
  ...
  const onTouchMove = (e) => {
    ...
    dragX.current = e.touches[0].clientX - touchX.current;
    setDrag(dragX.current);
  };
  const onTouchEnd = () => {
    if (Math.abs(dragX.current) > 60) go(dragX.current > 0 ? -1 : 1);
    touchX.current = null; dragX.current = 0; setDrag(0);
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 text-xs" style={{ color: 'rgba(255,255,255,0.7)' }}>
        {index + 1} / {images.length}
      </div>

      <div
        className="w-full flex-1 flex items-center justify-center overflow-hidden touch-pan-y"
      ...
      >
        <img
          ...
          style={{
            ...
            transition: touchX.current ? 'none' : 'transform 0.2s ease',
          }}
        />
      </div>
      ...
      {images.length > 1 && (
        <div className="flex items-center gap-6 mt-4" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => go(1)} disabled={index >= images.length - 1}
            style={{ color: '#fff', opacity: index >= images.length - 1 ? 0.3 : 1 }}>
            <ChevronRight size={26} />
          </button>
          <button onClick={() => go(-1)} disabled={index <= 0}
            style={{ color: '#fff', opacity: index <= 0 ? 0.3 : 1 }}>
            <ChevronLeft size={26} />
          </button>
        </div>
      )}
...
=========================================================== */
function ChatScreen({ c, coupleId, me, partner, bgUrl }) {
  ...
  const onImageChosen = async (e) => {
    ...
    try {
      ...
      const path = await uploadToBucket('chat-media', coupleId, optimized, 'img-');
  ...
  // ordered list of resolved image urls in this chat, for the swipeable lightbox
  const chatImageUrls = useMemo(
    () => messages.filter((m) => m.type === 'image' && mediaUrls[m.media_path]).map((m) => mediaUrls[m.media_path]),
    [messages, mediaUrls]
  );
  ...
  return (
    <div className="h-full flex flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 relative" style={{
        background: bgUrl
          ? `linear-gradient(${c.bg}cc, ${c.bg}cc), url(${bgUrl}) center/cover no-repeat`
          : c.bg,
      }}>
              ...
              <div style={{
                ...
                {m.type === 'image' && (
                  mediaUrls[m.media_path]
                    ? <img src={mediaUrls[m.media_path]} alt="" loading="lazy" className="w-40 h-28 rounded-xl object-cover cursor-pointer"
                        onClick={(e) => { e.stopPropagation(); setLightboxIdx(chatImageUrls.indexOf(mediaUrls[m.media_path])); }} />
      ...
      {uploading && (
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 text-xs px-3 py-1.5 rounded-full"
          style={{ background: c.bg2, color: c.gold, border: `1px solid ${c.border}` }}>...جاري إرسال الصورة</div>
      )}

      {lightboxIdx !== null && (
        <ImageLightbox images={chatImageUrls} startIndex={lightboxIdx} onClose={() => setLightboxIdx(null)} />
      )}
    </div>
  );
}
...
function MemoriesScreen({ c, coupleId, me, partner }) {
  ...
  // resolve signed urls for memory photos (mirrors the chat's media resolution)
  useEffect(() => {
    const need = memories.filter((m) => m.photo_path && !photoUrls[m.photo_path]);
    if (!need.length) return;
    (async () => {
      const entries = await Promise.all(need.map(async (m) => [m.photo_path, await signedUrl('memories', m.photo_path)]));
      setPhotoUrls((prev) => ({ ...prev, ...Object.fromEntries(entries) }));
    })();
  }, [memories, photoUrls]);

  const galleryUrls = useMemo(
    () => sorted.filter((m) => m.photo_path && photoUrls[m.photo_path]).map((m) => photoUrls[m.photo_path]),
    [sorted, photoUrls]
  );
  ...
  const add = async (title, date, note, file) => {
    ...
    if (file) {
      const optimized = await compressImage(file);
      photo_path = await uploadToBucket('memories', coupleId, optimized, 'mem-');
    }
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
            ...
            <div style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-4">
              {m.photo_path && (
                photoUrls[m.photo_path]
                  ? <img src={photoUrls[m.photo_path]} alt="" loading="lazy"
                      className="w-full h-40 rounded-xl object-cover mb-3 cursor-pointer"
                      onClick={() => setLightboxIdx(galleryUrls.indexOf(photoUrls[m.photo_path]))} />
                  : <div style={{ background: c.bg3 }} className="w-full h-40 rounded-xl mb-3 flex items-center justify-center"><ImageIcon size={26} style={{ color: c.textDim }} /></div>
              )}
      ...
      {lightboxIdx !== null && (
        <ImageLightbox images={galleryUrls} startIndex={lightboxIdx} onClose={() => setLightboxIdx(null)} />
      )}
    </div>
  );
}
...
=========================================================== */
function SettingsScreen({ c, dark, setDark, lang, setLang, me, coupleId, couple, partner }) {
  const [phrases] = useCoupleTable('phrases', coupleId);
  const [newPhrase, setNewPhrase] = useState('');
  const [bgBusy, setBgBusy] = useState(false);
  const bgInputRef = useRef(null);

  const onBgChosen = async (e) => {
    const file = e.target.files?.[0];
    e.target.value = '';
    if (!file || bgBusy) return;
    setBgBusy(true);
    try {
      const optimized = await compressImage(file, 1400, 0.75);
      const path = await uploadToBucket('memories', coupleId, optimized, 'bg-');
      await supabase.from('couples').update({ background_path: path }).eq('id', coupleId);
    } finally {
      setBgBusy(false);
    }
  };
  const removeBg = async () => {
    setBgBusy(true);
    try { await supabase.from('couples').update({ background_path: null }).eq('id', coupleId); }
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
      ...
      <SettingsBlock c={c} title="خلفية التطبيق (مشتركة بينكم)">
        <input ref={bgInputRef} type="file" accept="image/*" className="hidden" onChange={onBgChosen} />
        {couple?.background_path ? (
          <div className="flex items-center gap-2">
            <button onClick={() => bgInputRef.current?.click()} disabled={bgBusy}
              style={{ background: c.gold, color: c.bg, opacity: bgBusy ? 0.6 : 1 }}
              className="flex-1 rounded-full py-2.5 text-sm font-bold flex items-center justify-center gap-2">
              <ImagePlus size={15} /> {bgBusy ? '...' : 'تغيير الصورة'}
            </button>
            <button onClick={removeBg} disabled={bgBusy} style={{ color: c.rose, border: `1px solid ${c.rose}` }}
              className="rounded-full p-2.5"><Trash2 size={15} /></button>
          </div>
        ) : (
          <button onClick={() => bgInputRef.current?.click()} disabled={bgBusy}
            style={{ background: c.goldSoft, color: c.gold, opacity: bgBusy ? 0.6 : 1 }}
            className="w-full rounded-full py-2.5 text-sm font-bold flex items-center justify-center gap-2">
            <ImagePlus size={15} /> {bgBusy ? '...جاري الرفع' : 'أضف صورة خلفية'}
          </button>
        )}
        <div style={{ color: c.textDim }} className="text-xs mt-2">تظهر هذه الصورة كخلفية للتطبيق كله عند الاثنين، بما فيه الدردشة.</div>
      </SettingsBlock>
...
function GiffanTab({ c, coupleId, me, partner }) {
  ...
  const onGifChosen = async (e) => {
    const file = e.target.files?.[0];
    e.target.value = '';
    if (!file || sendingGif) return;
    setSendingGif(true);
    try {
      const path = await uploadToBucket('chat-media', coupleId, file, 'gif-');
      await supabase.from('messages').insert({ couple_id: coupleId, sender_id: me.id, type: 'image', media_path: path });
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'message', body: `${me.display_name} أرسل گيف من گيفان 🎬` });
    } finally {
      setSendingGif(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4 gap-2">
        <div style={{ color: c.textDim }} className="text-xs">خمّن معنى الكلمة الحسانية — نتيجة مشتركة ❤️</div>
        <button onClick={() => gifInputRef.current?.click()} disabled={sendingGif}
          style={{ background: c.goldSoft, color: c.gold, opacity: sendingGif ? 0.6 : 1 }}
          className="shrink-0 rounded-full px-3 py-1.5 text-xs font-bold flex items-center gap-1">
          <Film size={13} /> {sendingGif ? '...' : `أرسل گيف${partner ? ' لـ ' + partner.display_name : ''}`}
        </button>
      </div>
      {!finished ? (
        <>
        ...
        {stories.map((s) => (
          <div key={s.id} style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-4">
            <div className="flex items-center justify-between mb-1">
              <div style={{ fontFamily: FONT_DISPLAY, color: c.gold }} className="text-lg">{s.title}</div>
              <button
                onClick={async () => {
                  await supabase.from('messages').insert({ couple_id: coupleId, sender_id: me.id, type: 'text', text: `📖 ${s.title}\n${s.text}` });
                  if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'message', body: `${me.display_name} شارك قصة: ${s.title}` });
                }}
                title="شاركها في الدردشة" style={{ color: c.gold }} className="shrink-0">
                <Share2 size={16} />
              </button>
            </div>
...
export default function MesahaApp() {
  ...
  // shared app background image (couple-level, set from Settings by either partner)
  const [bgUrl, setBgUrl] = useState(null);
  useEffect(() => {
    if (!couple?.background_path) { setBgUrl(null); return; }
    let cancelled = false;
    signedUrl('memories', couple.background_path).then((url) => { if (!cancelled) setBgUrl(url); });
    return () => { cancelled = true; };
  }, [couple?.background_path]);

  // live-update the couple row (so a background set on one phone shows up on the other)
  useEffect(() => {
    if (!couple?.id) return;
    const ch = supabase.channel(`couple-${couple.id}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'couples', filter: `id=eq.${couple.id}` },
        (payload) => setCouple(payload.new))
      .subscribe();
    return () => supabase.removeChannel(ch);
  }, [couple?.id]);
    ...
    body = (
      <div className="h-full flex flex-col">
        <div className="flex-1 min-h-0">
          ...
          {tab === 'chat' && <ChatScreen c={c} coupleId={couple.id} me={me} partner={partner} bgUrl={bgUrl} />}
          ...
          {tab === 'settings' && <SettingsScreen c={c} dark={dark} setDark={() => {}} lang={lang} setLang={() => {}} me={me} coupleId={couple.id} couple={couple} partner={partner} />}
        </div>
        <div style={{ background: bgUrl ? `${c.bg2}f2` : c.bg2, borderTop: `1px solid ${c.border}` }} className="flex items-center justify-around py-2 px-1">
          {NAV.map((n) => (
  ...
  return (
    <div dir="rtl" style={{ fontFamily: FONT_BODY }} className="w-full h-screen flex items-center justify-center">
      ...
      <div style={{
        ...
        background: bgUrl ? `url(${bgUrl}) center/cover no-repeat` : c.bg,

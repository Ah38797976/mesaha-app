...
function ChatScreen({ c, coupleId, me, partner }) {
  ...
  const deleteMsg = async (id) => {
    ...
    setMessages((m) => m.filter((x) => x.id !== id)); // optimistic: remove instantly, don't wait for realtime echo
    const { error } = await supabase.from('messages').delete().eq('id', id);
    if (error) { loadMessages(); /* revert on failure */ }
  };
...
function MemoriesScreen({ c, coupleId, me, partner }) {
  ...
  // resolve signed urls for any memory that has a stored photo
  useEffect(() => {
    const need = sorted.filter((m) => m.photo_path && !photoUrls[m.photo_path]);
    if (!need.length) return;
    (async () => {
  ...
  const add = async (title, date, note, file) => {
    ...
    try {
      ...
      alert('تعذّر حفظ الذكرى، حاول مرة أخرى');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
            ...
            <div style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-4">
              {m.photo_path && (
                photoUrls[m.photo_path]
                  ? <img src={photoUrls[m.photo_path]} alt={m.title} className="w-full h-40 object-cover rounded-xl mb-2" />
                  : <div style={{ background: c.bg3 }} className="w-full h-40 rounded-xl mb-2 flex items-center justify-center"><ImageIcon size={26} style={{ color: c.textDim }} /></div>
              )}
      ...
      {showAdd && <AddMemoryModal c={c} onClose={() => setShowAdd(false)} onAdd={add} saving={saving} />}
    </div>
  );
}

function AddMemoryModal({ c, onClose, onAdd, saving }) {
  const todayStr = new Date().toISOString().slice(0, 10);
  const [title, setTitle] = useState('');
  const [date, setDate] = useState(todayStr); // default to today so the field is never invisibly empty
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div style={{ background: c.bg2, maxWidth: 420, width: '100%' }} className="rounded-t-3xl p-5">
        ...
        <div style={{ color: c.textDim }} className="text-xs mb-1">تاريخ الذكرى</div>
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
          style={{ background: c.bg3, color: c.text, border: `1px solid ${c.border}`, colorScheme: 'dark' }} className="w-full rounded-xl p-2 text-sm outline-none mb-3" />
        ...
        <button disabled={!title || !date || saving} onClick={() => onAdd(title, date, note, file)}
          style={{ background: c.gold, color: c.bg, opacity: (!title || !date || saving) ? 0.6 : 1 }}
          className="w-full rounded-full py-3 font-bold">{saving ? 'جارِ الحفظ...' : 'حفظ الذكرى'}</button>
      </div>
    </div>
  );
}
...
/* ===========================================================
   5b) GALLERY — quick shared photo grid (no title/date required)
=========================================================== */
function GalleryScreen({ c, coupleId, me, partner }) {
  const [photos] = useCoupleTable('gallery_photos', coupleId, 'created_at');
  ...
  useEffect(() => {
    const need = sorted.filter((p) => p.photo_path && !urls[p.photo_path]);
    if (!need.length) return;
    (async () => {
      const entries = await Promise.all(need.map(async (p) => [p.photo_path, await signedUrl('memories', p.photo_path)]));
  ...
  const onFilesChosen = async (e) => {
    ...
    try {
      for (const file of files) {
        const photo_path = await uploadToBucket('memories', coupleId, file, 'gal-');
        await supabase.from('gallery_photos').insert({ couple_id: coupleId, added_by: me.id, photo_path });
      }
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'memory', body: `📸 أضاف ${me.display_name} صورة للمعرض` });
    } catch (err) {
  ...
  const removePhoto = async (p) => {
    ...
    if (error) alert('تعذّر حذف الصورة');
  };
  ...
  return (
    <div className="h-full overflow-y-auto px-4 py-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div style={{ fontFamily: FONT_DISPLAY, color: c.gold }} className="text-xl">المعرض 🖼️</div>
        <button onClick={pick} disabled={uploading} style={{ background: c.goldSoft, color: c.gold, opacity: uploading ? 0.6 : 1 }} className="w-9 h-9 rounded-full flex items-center justify-center">
          {uploading ? <span className="text-[10px]">...</span> : <Plus size={18} />}
        </button>
      </div>

      {!sorted.length && (
        <div style={{ color: c.textDim }} className="text-sm text-center mt-16">
          ما فيه صور بعد — اضغط + لإضافة أول صورة 📸
        </div>
      )}

      <div className="grid grid-cols-3 gap-1.5">
        {sorted.map((p) => (
          <button key={p.id} onClick={() => urls[p.photo_path] && setViewerPhoto(p)} style={{ background: c.bg3 }} className="aspect-square rounded-lg overflow-hidden">
            {urls[p.photo_path]
              ? <img src={urls[p.photo_path]} alt="" className="w-full h-full object-cover" />
              : <div className="w-full h-full flex items-center justify-center"><ImageIcon size={18} style={{ color: c.textDim }} /></div>}
          </button>
        ))}
      </div>

      {viewerPhoto && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center" style={{ background: 'rgba(0,0,0,0.92)' }} onClick={() => setViewerPhoto(null)}>
          <button onClick={() => setViewerPhoto(null)} className="absolute top-5 left-5"><X size={22} color="#fff" /></button>
          <img src={urls[viewerPhoto.photo_path]} alt="" className="max-w-[92vw] max-h-[75vh] object-contain rounded-xl" onClick={(e) => e.stopPropagation()} />
          <div className="flex items-center gap-4 mt-5">
            <span style={{ color: '#fff9' }} className="text-xs">{timeAgo(viewerPhoto.created_at)}</span>
            {viewerPhoto.added_by === me.id && (
              <button onClick={(e) => { e.stopPropagation(); removePhoto(viewerPhoto); }} style={{ color: '#F1A' }} className="text-xs underline">حذف</button>
            )}
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div style={{ background: c.bg2, maxWidth: 420, width: '100%' }} className="rounded-t-3xl p-5">
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="اسم المناسبة"
          style={{ background: c.bg3, color: c.text, border: `1px solid ${c.border}` }} className="w-full rounded-xl p-2 text-sm outline-none mb-3" />
        <div style={{ color: c.textDim }} className="text-xs mb-1">تاريخ المناسبة</div>
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
          style={{ background: c.bg3, color: c.text, border: `1px solid ${c.border}`, colorScheme: 'dark' }} className="w-full rounded-xl p-2 text-sm outline-none mb-4" />
        <button disabled={!title || !date} onClick={() => onAdd(title, date)}
          style={{ background: c.gold, color: c.bg, opacity: (!title || !date) ? 0.6 : 1 }}
          className="w-full rounded-full py-3 font-bold">إضافة</button>
      </div>
    </div>
  );
}
...
function SettingsScreen({ c, dark, setDark, lang, setLang, me, coupleId, partner }) {
  const [phrases, , setPhrases] = useCoupleTable('phrases', coupleId);
  ...
  const removePhrase = async (id) => { setPhrases((p) => p.filter((x) => x.id !== id)); await supabase.from('phrases').delete().eq('id', id); };
...
export default function MesahaApp() {
  ...
  // optimistic local update so the toggle feels instant instead of waiting on the realtime round-trip
  const setDark = useCallback((v) => setMe((prev) => (prev ? { ...prev, dark_mode: v } : prev)), []);
  const setLang = useCallback((v) => setMe((prev) => (prev ? { ...prev, lang: v } : prev)), []);
    ...
    body = (
      <div className="h-full flex flex-col">
        <div className="flex-1 min-h-0">
          ...
          {tab === 'gallery' && <GalleryScreen c={c} coupleId={couple.id} me={me} partner={partner} />}

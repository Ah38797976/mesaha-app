...
import { supabase, uploadToBucket, uploadBlob, signedUrl, notifyPartner, removeFromBucket, SUPABASE_CONFIG_ERROR } from './lib/supabaseClient';
...
function CoupleSetupScreen({ c, userId, onLinked }) {
  ...
  const join = async () => {
    setErr(''); setBusy(true);
    try {
      // Looking up a couple by invite code has to happen BEFORE the caller
      // is a member, so a plain `.from('couples').select(...)` can never
      // work here: RLS on `couples` only allows a member (or the creator)
      // to read the row, so every code — valid or not — looked invalid.
      // This RPC is a SECURITY DEFINER function that returns *only* the
      // couple id for a matching code (see
      // supabase/migration_stageA_fix_invite_join.sql), so it fixes the
      // lookup without exposing the couples table itself.
      const { data: coupleId, error } = await supabase
        .rpc('find_couple_id_by_invite_code', { p_code: code.trim() });
      if (error) throw error;
      if (!coupleId) throw new Error('كود الدعوة غير صحيح');
      const { error: mErr } = await supabase.from('couple_members').insert({ couple_id: coupleId, user_id: userId });
      if (mErr) throw mErr;
      onLinked();
    } catch (e) {
  ...
  return (
    <div className="h-full overflow-y-auto relative">
      ...
      <div className="px-5 -mt-2 relative z-10">
        ...
        {onOpenBayna && <BaynaHomeCard c={c} onOpen={onOpenBayna} />}
...
function ChatScreen({ c, coupleId, me, partner }) {
  ...
  const [sendingMedia, setSendingMedia] = useState(false); // true while an image or voice note is uploading
  const [chatError, setChatError] = useState('');
  const bottomRef = useRef(null);
  const fileInputRef = useRef(null);
  const mediaRecorderRef = useRef(null);
  const chunksRef = useRef([]);
  const CHAT_MAX_IMAGE_BYTES = 20 * 1024 * 1024; // 20MB, same ceiling as Gallery

  const flashChatError = (msg) => {
    setChatError(msg);
    setTimeout(() => setChatError((m) => (m === msg ? '' : m)), 3500);
  };
  ...
  const pickImage = () => { if (!sendingMedia) fileInputRef.current?.click(); };
  const onImageChosen = async (e) => {
    const file = e.target.files?.[0];
    e.target.value = '';
    if (!file || sendingMedia) return;
    if (!file.type?.startsWith('image/')) { flashChatError('نوع الملف غير مدعوم — اختر صورة'); return; }
    if (file.size > CHAT_MAX_IMAGE_BYTES) { flashChatError('حجم الصورة أكبر من 20 ميغابايت'); return; }
    setSendingMedia(true);
    let path = null;
    try {
      path = await uploadToBucket('chat-media', coupleId, file, 'img-');
      const { error } = await supabase.from('messages').insert({ couple_id: coupleId, sender_id: me.id, type: 'image', media_path: path });
      if (error) { await removeFromBucket('chat-media', path); throw error; }
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'message', body: `${me.display_name} أرسل صورة 📸` });
    } catch (err) {
      console.error('chat image send failed:', err?.message || err);
      flashChatError('تعذّر إرسال الصورة، حاول مجددًا');
    } finally {
  ...
  const startRecording = async () => {
    if (sendingMedia || recording) return;
    let stream;
    try {
      stream = await navigator.mediaDevices.getUserMedia({ audio: true });
    } catch (err) {
      flashChatError('تعذّر الوصول للميكروفون — تحقق من إذن المتصفح');
      return;
    }
    ...
    rec.onstop = async () => {
      ...
      let path = null;
      try {
        path = await uploadBlob('voice-notes', coupleId, blob, 'webm', 'voice-');
        const { error } = await supabase.from('messages').insert({ couple_id: coupleId, sender_id: me.id, type: 'voice', media_path: path });
        if (error) { await removeFromBucket('voice-notes', path); throw error; }
        ...
        console.error('chat voice send failed:', err?.message || err);
        flashChatError('تعذّر إرسال التسجيل الصوتي، حاول مجددًا');
      } finally {
        setSendingMedia(false);
      }
    };
  ...
  return (
    <div className="h-full flex flex-col">
      <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={onImageChosen} disabled={sendingMedia} />
      ...
      {chatError && (
        <div className="px-4 py-2 text-center" style={{ background: c.bg2 }}>
          <span style={{ color: c.rose }} className="text-xs">{chatError}</span>
        </div>
      )}

      <div style={{ background: c.bg2, borderTop: `1px solid ${c.border}` }} className="px-3 py-3 flex items-center gap-2">
        <button onClick={pickImage} disabled={sendingMedia} style={{ color: c.gold, opacity: sendingMedia ? 0.5 : 1 }}><ImageIcon size={20} /></button>
        <button onClick={recording ? stopRecording : startRecording} disabled={sendingMedia && !recording}
          style={{ color: recording ? c.rose : c.gold, opacity: sendingMedia && !recording ? 0.5 : 1 }}>
...
function MemoriesScreen({ c, coupleId, me, partner }) {
  ...
  // resolve signed urls for memory photos (photo_path was being saved but
  // never turned into a displayable url, so photos never rendered)
  useEffect(() => {
  ...
  const add = async (title, date, note, file) => {
    if (addBusy) return; // منع الضغط المتكرر على زر الحفظ
    setAddBusy(true);
    setAddError('');
    let photo_path = null;
    try {
      if (file) photo_path = await uploadToBucket('memories', coupleId, file, 'mem-');
      const { error } = await supabase.from('memories').insert({ couple_id: coupleId, added_by: me.id, title, memory_date: date, note, photo_path });
      if (error) {
        if (photo_path) await removeFromBucket('memories', photo_path); // لا نترك صورة يتيمة
        throw error;
      }
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'memory', body: `📸 أضاف ${me.display_name} ذكرى` });
      setShowAdd(false);
    } catch (err) {
      console.error('add memory failed:', err?.message || err);
      setAddError('تعذّر حفظ الذكرى، حاول مجددًا');
    } finally {
      setAddBusy(false);
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
            ...
            <div style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-4">
              {m.photo_path && (
                photoUrls[m.photo_path]
                  ? <img src={photoUrls[m.photo_path]} alt="" className="w-full h-40 object-cover rounded-xl mb-2" />
                  : <div style={{ background: c.bg3 }} className="w-full h-40 rounded-xl mb-2 flex items-center justify-center"><ImageIcon size={22} style={{ color: c.textDim }} /></div>
              )}
      ...
      {showAdd && <AddMemoryModal c={c} onClose={() => setShowAdd(false)} onAdd={add} busy={addBusy} error={addError} />}
    </div>
  );
}

function AddMemoryModal({ c, onClose, onAdd, busy, error }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div style={{ background: c.bg2, maxWidth: 420, width: '100%' }} className="rounded-t-3xl p-5">
        ...
        <input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] || null)} disabled={busy} className="w-full text-xs mb-2" style={{ color: c.textDim }} />
        {error && <div style={{ color: c.rose }} className="text-xs mb-3">{error}</div>}
        <button disabled={busy || !title || !date} onClick={() => onAdd(title, date, note, file)}
          style={{ background: c.gold, color: c.bg, opacity: (busy || !title || !date) ? 0.6 : 1 }}
          className="w-full rounded-full py-3 font-bold">{busy ? '...جاري الحفظ' : 'حفظ الذكرى'}</button>
      </div>
    </div>
  );
}
...
function HomeWithPhrases({ c, t, me, partner, couple, onQuickAction, onOpenChat, onOpenBayna, toasts }) {
  const [phrases] = useCoupleTable('phrases', couple.id);
  return <HomeScreen c={c} t={t} me={me} partner={partner} couple={couple} phrases={phrases} onQuickAction={onQuickAction} onOpenChat={onOpenChat} onOpenBayna={onOpenBayna} toasts={toasts} />;
}
...
export default function MesahaApp() {
  // Throws inside the component tree (not at module scope) so the
  // ErrorBoundary in main.jsx can catch it and show a friendly message +
  // retry button instead of a blank white page. See the explanation in
  // lib/supabaseClient.js next to SUPABASE_CONFIG_ERROR.
  ...
  const loadCouple = useCallback(async (userId) => {
    ...
    try {
      ...
      // Log instead of failing silently. Couple/partner are left as-is (not
      // wiped) so a transient network error doesn't kick the user out of a
      // space they were already in.
  ...
  useEffect(() => {
    ...
    if (!session) { setMe(null); setCouple(null); setPartner(null); setCoupleLoading(false); return; }
  ...
  let body;
  if (session === undefined || (session && !me) || (session && coupleLoading)) {

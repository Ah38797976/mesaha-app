...
// Resolves a private-bucket storage path to a temporary signed URL for <img>/<video> display.
// Returns null while loading or if there's no path yet.
function useSignedUrl(bucket, path) {
  const [url, setUrl] = useState(null);
  useEffect(() => {
    let cancelled = false;
    if (!path) { setUrl(null); return; }
    signedUrl(bucket, path).then((u) => { if (!cancelled) setUrl(u); });
...
function HomeScreen({ c, t, me, partner, couple, phrases, onQuickAction, onOpenChat, toasts }) {
  ...
  const myAvatar = useSignedUrl('avatars', me?.avatar_url);
  const partnerAvatar = useSignedUrl('avatars', partner?.avatar_url);
        ...
        <div className="absolute inset-x-0 bottom-0 flex flex-col items-center pb-4">
          <div className="flex items-center -space-x-3 rtl:space-x-reverse mb-3">
            <div style={{ background: c.gold, color: c.bg, fontFamily: FONT_DISPLAY }}
              className="w-16 h-16 rounded-full flex items-center justify-center text-xl border-4 overflow-hidden">
              {myAvatar ? <img src={myAvatar} alt="" className="w-full h-full object-cover" /> : (me?.display_name?.[0] || '؟')}
            </div>
            ...
            <div style={{ background: c.rose, color: c.bg, fontFamily: FONT_DISPLAY }}
              className="w-16 h-16 rounded-full flex items-center justify-center text-xl border-4 overflow-hidden">
              {partnerAvatar ? <img src={partnerAvatar} alt="" className="w-full h-full object-cover" /> : (partner?.display_name?.[0] || '؟')}
            </div>
          </div>
...
/* ===========================================================
   5b) ALBUM — simple shared photo grid, no titles/dates required
=========================================================== */
function AlbumScreen({ c, coupleId, me, partner }) {
  const [photos] = useCoupleTable('album_photos', coupleId);
  const [urls, setUrls] = useState({});
  const [uploading, setUploading] = useState(false);
  const fileInputRef = useRef(null);

  useEffect(() => {
    const need = photos.filter((p) => p.photo_path && !urls[p.photo_path]);
    if (!need.length) return;
    (async () => {
      const entries = await Promise.all(
        need.map(async (p) => [p.photo_path, await signedUrl('album-photos', p.photo_path)])
      );
  ...
  const onChosen = async (e) => {
    ...
    setUploading(true);
    try {
      const path = await uploadToBucket('album-photos', coupleId, file, 'album-');
      await supabase.from('album_photos').insert({ couple_id: coupleId, added_by: me.id, photo_path: path });
      if (partner) notifyPartner({ coupleId, recipientId: partner.id, actorId: me.id, kind: 'album', body: `📷 أضاف ${me.display_name} صورة للألبوم` });
    } catch (err) {
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
      <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={onChosen} />
      <div className="flex items-center justify-between mb-4">
        <div style={{ fontFamily: FONT_DISPLAY, color: c.gold }} className="text-xl">ألبومنا 🖼️</div>
        <button onClick={pick} disabled={uploading} style={{ background: c.goldSoft, color: c.gold }}
          className="w-9 h-9 rounded-full flex items-center justify-center">
          <Plus size={18} />
        </button>
      </div>
      {uploading && <div style={{ color: c.textDim }} className="text-xs mb-3">...جاري رفع الصورة</div>}
      {photos.length === 0 ? (
        <div style={{ color: c.textDim }} className="text-sm text-center mt-10">ما فيه صور بعد — اضغطي (+) عشان تضيفين أول صورة</div>
      ) : (
        <div className="grid grid-cols-3 gap-2">
          {[...photos].reverse().map((p) => (
            <div key={p.id} style={{ background: c.bg2 }} className="aspect-square rounded-xl overflow-hidden">
              {urls[p.photo_path] ? (
...
=========================================================== */
function SettingsScreen({ c, dark, setDark, lang, setLang, me, coupleId, partner, setCoupleBackground }) {
  ...
  const myAvatarUrl = useSignedUrl('avatars', me.avatar_url);
  ...
  const onAvatarChosen = async (e) => {
    ...
    setAvatarBusy(true);
    try {
      const path = await uploadToBucket('avatars', coupleId, file, `${me.id}-`);
  ...
  const onBgChosen = async (e) => {
    ...
    setBgBusy(true);
    try {
      const path = await uploadToBucket('backgrounds', coupleId, file, 'bg-');
      await supabase.from('couples').update({ background_url: path }).eq('id', coupleId);
      setCoupleBackground?.(path);
    } catch (err) {
      alert(err.message || 'تعذر رفع صورة الخلفية');
    } finally {
      setBgBusy(false);
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
      ...
      <SettingsBlock c={c} title="صورتك الشخصية">
        <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={onAvatarChosen} />
        <div className="flex items-center gap-3">
          <div style={{ background: c.bg3, fontFamily: FONT_DISPLAY, color: c.gold }}
            className="w-14 h-14 rounded-full overflow-hidden flex items-center justify-center text-lg">
            {myAvatarUrl ? <img src={myAvatarUrl} alt="" className="w-full h-full object-cover" /> : (me.display_name?.[0] || '؟')}
          </div>
          <button onClick={() => avatarInputRef.current?.click()} disabled={avatarBusy}
            style={{ color: c.gold, border: `1px solid ${c.gold}` }} className="rounded-full px-4 py-2 text-xs">
      ...
      <SettingsBlock c={c} title="خلفية التطبيق (مشتركة بينكم)">
        ...
        <button onClick={() => bgInputRef.current?.click()} disabled={bgBusy}
          style={{ color: c.gold, border: `1px solid ${c.gold}` }}
          className="w-full rounded-full py-2.5 text-xs flex items-center justify-center gap-2">
          <ImageIcon size={14} /> {bgBusy ? '...جاري الرفع' : 'اختيار صورة خلفية'}
        </button>
      </SettingsBlock>
...
export default function MesahaApp() {
  ...
  // live-update the shared couple row (e.g. background_url changed by either partner)
  useEffect(() => {
    if (!couple) return;
  ...
  // Same idea for the shared app-background image, set from Settings.
  const setCoupleBackground = useCallback((path) => setCouple((cur) => (cur ? { ...cur, background_url: path } : cur)), []);
  const bgUrl = useSignedUrl('backgrounds', couple?.background_url);
  ...
  const NAV = [
    ...
    { key: 'album', label: 'ألبومنا', icon: ImageIcon },
    ...
    body = (
      <div className="h-full flex flex-col">
        <div className="flex-1 min-h-0">
          ...
          {tab === 'album' && <AlbumScreen c={c} coupleId={couple.id} me={me} partner={partner} />}
          {tab === 'occasions' && <OccasionsScreen c={c} coupleId={couple.id} me={me} partner={partner} />}
          {tab === 'settings' && <SettingsScreen c={c} dark={dark} setDark={setDark} lang={lang} setLang={setLang} me={me} coupleId={couple.id} partner={partner} setCoupleBackground={setCoupleBackground} />}
        </div>

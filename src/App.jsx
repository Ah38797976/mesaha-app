...
function ChatScreen({ c, coupleId, me, partner }) {
  ...
  const startRecording = async () => {
    ...
    // iPhone/Safari's MediaRecorder does NOT support the 'audio/webm' container
    // (it only records audio/mp4). Forcing 'audio/webm' regardless of the
    // device meant Safari silently recorded mp4 data into a file we then
    // labeled and saved as .webm — so on playback the browser couldn't
    // decode it and showed "خطأ" instead of the audio player. Ask the
    // browser what it can actually record and use that.
    ...
    const rec = supportedType ? new MediaRecorder(stream, { mimeType: supportedType }) : new MediaRecorder(stream);
    chunksRef.current = [];
    rec.ondataavailable = (e) => chunksRef.current.push(e.data);
    rec.onstop = async () => {
      const actualType = rec.mimeType || supportedType || 'audio/webm';
...
export default function MesahaApp() {
  ...
  const NAV = [
    ...
    { key: 'gallery', label: 'صور تجمعنا', icon: ImageIcon },

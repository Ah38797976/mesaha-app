...
/* ---------------------------------------------------------
   نفس الموجة 🌊 — multiple-choice guess/match game.
   Unlike «من الأكثر» (binary: pick which of you two), each question here
   has its own 3-4 answer options; both partners pick independently and it
   reveals whether you matched, with a running score across the round.
--------------------------------------------------------- */
function MatchQuizTab({ c, coupleId, me, partner }) {
  const questions = useBaynaContent('question', 'match_quiz', null);
  const [idx, setIdx] = useState(0);
  const [rows, setRows] = useState([]);
  const [awarded, setAwarded] = useState(false);
  const [session, setSession] = useState({ played: 0, matched: 0 });
  const [finished, setFinished] = useState(false);
  const question = questions[idx];

  const load = useCallback(async () => {
    if (!question) { setRows([]); return; }
    const { data } = await supabase.from('bayna_match_answers').select('*').eq('couple_id', coupleId).eq('question_id', question.id);
    setRows(data || []);
  }, [coupleId, question]);

  useEffect(() => { load(); setAwarded(false); }, [load]);
  useEffect(() => {
    const ch = supabase.channel(`bayna-match-${coupleId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bayna_match_answers', filter: `couple_id=eq.${coupleId}` }, () => load())
      .subscribe();
    return () => supabase.removeChannel(ch);
  }, [coupleId, load]);

  if (!questions.length) return <EmptyState c={c} text="ماكاين أسئلة حالياً — أضف أسئلة من لوحة الإدارة." />;

  const mine = rows.find((r) => r.user_id === me.id);
  const theirs = partner && rows.find((r) => r.user_id === partner.id);
  const bothAnswered = !!(mine && theirs);
  const matched = bothAnswered && mine.choice_text === theirs.choice_text;

  useEffect(() => {
    if (bothAnswered && !awarded) {
      setAwarded(true);
      setSession((s) => ({ played: s.played + 1, matched: s.matched + (matched ? 1 : 0) }));
      if (matched) awardPoints(coupleId, me.id, 10);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bothAnswered]);

  const choose = async (optionText) => {
    if (mine) return;
    await supabase.from('bayna_match_answers').upsert(
      { couple_id: coupleId, question_id: question.id, user_id: me.id, choice_text: optionText },
      { onConflict: 'couple_id,question_id,user_id' },
    );
    load();
  };
  ...
  const playAgain = () => { setSession({ played: 0, matched: 0 }); setFinished(false); setIdx(0); };

  if (finished) {
    const pct = session.played ? Math.round((session.matched / session.played) * 100) : 0;
    return (
      <div className="flex flex-col items-center text-center py-8">
        <div style={{ fontFamily: FONT_DISPLAY, color: c.gold }} className="text-xl mb-2">خلصتوا الجولة! 🌊</div>
        <div style={{ color: c.text }} className="text-sm mb-1">توافقتوا في {session.matched} من {session.played} سؤال</div>
        <div style={{ color: c.gold }} className="text-3xl font-bold mb-6">{pct}%</div>
        <button onClick={playAgain} style={{ background: c.gold, color: c.bg }} className="rounded-full px-6 py-2.5 text-sm font-bold">إعادة الجولة</button>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ color: c.textDim }} className="text-xs mb-3">
        سؤال {idx + 1} من {questions.length} {session.played > 0 && `· توافقتوا ${session.matched}/${session.played}`}
      </div>
      <div style={{ background: c.bg2, border: `1px solid ${c.gold}` }} className="rounded-3xl p-6 mb-4 text-center">
        <div style={{ fontFamily: FONT_DISPLAY, color: c.text }} className="text-xl leading-relaxed">{question.text}</div>
      </div>
      ...
      {!mine && (
        <div className="grid grid-cols-2 gap-3 mb-3">
          {(question.options || []).map((opt, i) => (
            <button key={i} onClick={() => choose(opt)} style={{ background: c.bg2, border: `1px solid ${c.border}`, color: c.text }}
              className="rounded-2xl py-4 px-2 text-sm font-bold">{opt}</button>
          ))}
        </div>
      )}

      {mine && !theirs && (
        <div style={{ background: c.goldSoft, color: c.text }} className="rounded-2xl p-4 text-center text-sm">اخترت "{mine.choice_text}" — بانتظار {partner?.display_name || 'شريكك'}…</div>
      )}
      ...
      {bothAnswered && (
        <div className="grid gap-3">
          <div className="grid grid-cols-2 gap-3">
            <div style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-3 text-center">
              <div style={{ color: c.gold }} className="text-xs font-bold mb-1">{me.display_name}</div>
              <div style={{ color: c.text }} className="text-sm">{mine.choice_text}</div>
            </div>
            <div style={{ background: c.bg2, border: `1px solid ${c.border}` }} className="rounded-2xl p-3 text-center">
              <div style={{ color: c.rose }} className="text-xs font-bold mb-1">{partner?.display_name}</div>
              <div style={{ color: c.text }} className="text-sm">{theirs.choice_text}</div>
            </div>
          </div>
          {matched
            ? <div style={{ background: 'rgba(120,200,140,0.18)', color: c.text }} className="rounded-2xl p-3 text-center text-sm font-bold">نفس الموجة! +10 نقاط 🌊</div>
            : <div style={{ background: 'rgba(220,100,100,0.12)', color: c.text }} className="rounded-2xl p-3 text-center text-sm">اختلفتوا هالمرة 😅</div>}
          <button onClick={next} style={{ background: c.gold, color: c.bg }} className="rounded-full py-3 font-bold text-sm">السؤال التالي</button>
        </div>
      )}
    </div>
...
export function BaynaScreen({ c, coupleId, me, partner }) {
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5">
      ...
      {sub === 'match' && <MatchQuizTab c={c} coupleId={coupleId} me={me} partner={partner} />}
...
const SUBTYPES_BY_TYPE = {
  question: [
    ...
    { key: 'match_quiz', label: 'نفس الموجة 🌊 (اختيار متعدد)' },
...
function emptyBaynaForm(type) {
  return { type, subtype: '', level: '', lang: 'ar', text: '', active: true, options: ['', '', '', ''] };
}
...
export function AdminBaynaScreen({ c, me }) {
  ...
  const startEdit = (row) => {
    ...
    setForm({ type: row.type, subtype: row.subtype || '', level: row.level || '', lang: row.lang, text: row.text, active: row.active, options: opts });
    setEditing(row.id);
  };

  const save = async () => {
    if (!form.text.trim()) return;
    const isMatchQuiz = form.type === 'question' && form.subtype === 'match_quiz';
    const cleanOptions = isMatchQuiz ? form.options.map((o) => o.trim()).filter(Boolean) : null;
    if (isMatchQuiz && cleanOptions.length < 2) return; // need at least 2 real choices
    const payload = {
      type: form.type, subtype: form.subtype || null, level: form.level || null, lang: form.lang,
      text: form.text.trim(), active: form.active, options: cleanOptions,
    };
    if (editing === 'new') {
      ...
      {editing && (
        <div style={{ background: c.bg2, border: `1px solid ${c.gold}` }} className="rounded-2xl p-4 mb-4">
          <div className="grid gap-3">
            ...
            {form.type === 'question' && form.subtype === 'match_quiz' && (
              <div className="grid gap-2">
                <div style={{ color: c.textDim }} className="text-xs">خيارات الإجابة (اثنين على الأقل، فاضي = يتجاهل)</div>
                {form.options.map((opt, i) => (
                  <input key={i} value={opt} onChange={(e) => setForm((f) => ({ ...f, options: f.options.map((o, j) => (j === i ? e.target.value : o)) }))}
                    placeholder={`خيار ${i + 1}`} style={{ background: c.bg3, color: c.text, border: `1px solid ${c.border}` }}

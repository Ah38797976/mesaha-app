...
export default function Gallery({ c, coupleId, me, partner }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const entries = await Promise.all(need.map(async (p) => [p.storage_path, await signedUrl('gallery-photos', p.storage_path)]));
  ...
  const doUpload = useCallback(async (blob) => {
    try {
      const path = await uploadWithProgress('gallery-photos', coupleId, blob, (progress) =>
  ...
  const deletePhoto = async (photo) => {
    ...
    supabase.storage.from('gallery-photos').remove([photo.storage_path]).then(() => {});
    setOpenId(null);
  };

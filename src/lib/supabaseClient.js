...
export async function signedUrl(bucket, path, expiresIn = 3600) {
  ...
  if (error) {
    ...
    // TEMP DEBUG — احذف هذا السطر بعد ما نلقى سبب المشكلة.
    // بيوريك رسالة الخطأ الحقيقية القادمة من Supabase مباشرة على الشاشة،
    // بدون ما تحتاج تفتح Console أو توصل الجهاز بـ Mac.
    if (typeof window !== 'undefined' && window.alert) {
      window.alert(`signedUrl error\nbucket: ${bucket}\npath: ${path}\n${error.message}`);
    }
    return null;
  }
